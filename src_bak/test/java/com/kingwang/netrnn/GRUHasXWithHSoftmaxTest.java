 ...
 * @version 1.0
 */
public class GRUHasXWithHSoftmaxTest {
	...
	private int[] clsRcd;
	private InputLayerHasCOut input;
	private GRU gru;
	private Attention att;
	private OutputLayerHasXWithHSoftMax output;
	...
	public void setCell() {
		...
		input = new InputLayerHasCOut(nodeSize, inDynSize, new MatIniter(Type.Test, 0, 0, 0));
		...
		output = new OutputLayerHasXWithHSoftMax(inDynSize, inFixedSize, outSize, AlgConsHSoftmax.cNum, new MatIniter(Type.Test, 0, 0, 0));
...
//	        acts.put("py" + t, predictYt);
	...
	private void gradientTestAndretActualGradient(List<String> nodes, DoubleMatrix mat
		...
		input.bptt(acts, targetT, output, gru);
	}
...
//		nodeVec.put(1, x);
		...
		input = new InputLayerHasCOut(nodeSize, inDynSize, initer);
		gru = new GRU(inDynSize, inFixedSize, outSize, initer);
		att = new Attention(outSize, initer);
		output = new OutputLayerHasXWithHSoftMax(inDynSize, inFixedSize, outSize, AlgConsHSoftmax.cNum, initer);
		int reviseLoc = 1;
		int targetT = 1;

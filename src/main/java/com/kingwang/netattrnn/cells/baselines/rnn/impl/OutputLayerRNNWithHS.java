 ...
 */
package com.kingwang.netattrnn.cells.baselines.rnn.impl;
 ...
 * @version 1.0
 */
public class OutputLayerRNNWithHS extends Operator implements Cell, Serializable {
    ...
    public OutputLayerRNNWithHS(int outSize, int cNum, MatIniter initer) {
    ...
    public void bptt(Map<String, DoubleMatrix> acts, int lastT, Cell... cell) {
    	...
    	for (int t = lastT; t > -1; t--) {
            ...
            DoubleMatrix s = acts.get("h" + t).transpose();
    ...
    public void updateParametersByAdam(BatchDerivative derv, double lr
		...
		hd2bsc = hd2bsc.mul(beta2).add(MatrixFunctions.pow(batchDerv.dbc, 2.).mul(1 - beta2));
		
		hdWsc = hdWsc.mul(beta1).add(batchDerv.dWsc.mul(1 - beta1));
		hdbsc = hdbsc.mul(beta1).add(batchDerv.dbc.mul(1 - beta1));

 ...
 * @version 1.0
 */
public class AttentionWithCov_bak extends Operator implements Cell, Serializable {
	...
	public AttentionWithCov_bak(int inDynSize, int inFixedSize, int attSize, int hiddenSize
	...
	public void active(int t, Map<String, DoubleMatrix> acts, double... params) {
		...
		for(int k=bsIdx; k<t+1; k++) { //TODO:here is the problem
			...
			DoubleMatrix gsk = null;
			if(k<t) {
				gsk = Activer.tanh(prevT.mmul(W).add(hk.mmul(U))
						.add(prevV.getRow(k-bsIdx+biasPos).mmul(Z)).add(bs));
			} else {
				gsk = Activer.tanh(prevT.mmul(W).add(hk.mmul(U)).add(bs));
			}
...
//            }
	...
	private void calcWeightsGradient(Map<String, DoubleMatrix> acts, int lastT) {
		...
		for(int t=0; t<lastT+1; t++) {
			...
			for(int k=bsIdx; k<t+1; k++) {
				...
				if(k<t) {
					dZ = dZ.add(prevV.getRow(k-bsIdx+biasPos).transpose().mmul(deltaGs.getRow(k-bsIdx)));
				}
	...
	public void updateParametersByAdaGrad(BatchDerivative derv, double lr) {
        ...
        hdW = hdW.add(MatrixFunctions.pow(batchDerv.dZ, 2.));

 ...
 * @version 1.0
 */
public class Attention_alphaReg extends Operator implements Cell, Serializable {
	...
	public Attention_alphaReg(int inDynSize, int inFixedSize, int attSize, int hiddenSize, MatIniter initer) {
...
//		acts.put("tanhGs"+t, tanhGs);
	...
	public void bptt(Map<String, DoubleMatrix> acts, int lastT, Cell... cell) {
		...
		for (int t = lastT; t > -1; t--) {
			...
			DoubleMatrix deltaAlphaReg = acts.get("dAlgReg"+t);
			//get cidx
			...
			for(int j=bsIdx; j<t+1; j++) {
				...
				deltaAlpha.put(j-bsIdx, deltaS.mmul(hj.transpose()).get(0)
						+AlgConsHSoftmax.lambda_alpha*alpha.get(j-bsIdx)); //TODO: add alpha regularization
			}
			// delta e

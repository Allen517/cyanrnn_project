 ...
 * @version 1.0
 */
public class AttentionWithCov_bak2 extends Operator implements Cell, Serializable {
	...
	public AttentionWithCov_bak2(int inDynSize, int inFixedSize, int attSize, int hiddenSize
	...
	private void calcWeightsGradient(Map<String, DoubleMatrix> acts, int lastT) {
		...
		for(int t=0; t<lastT+1; t++) {
			...
			for(int k=bsIdx; k<t+1; k++) {
				...
				dZ = dZ.add(vecV.getRow(k-bsIdx+biasPos).transpose().mmul(deltaGs.getRow(k-bsIdx)));

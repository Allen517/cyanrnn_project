 ...
 * @version 1.0
 */
public class Coverage_bak extends Operator implements Cell, Serializable {
	...
	public Coverage_bak(int covSize, int attSize, int hiddenSize, MatIniter initer) {
		...
		for(int k=bsIdx; k<t+1; k++) {
			...
			if(k<t) { //TODO:here is the problem
				vecV.putRow(k-bsIdx, Activer.logistic(prevV.getRow(k-bsIdx+biasPos).mmul(Wvv)
										.add(Wav.mul(alpha.get(k-bsIdx)))
										.add(hk.mmul(Whv)).add(prevT.mmul(Wtv)).add(bv)));
			} else {
				vecV.putRow(k-bsIdx, Activer.logistic(Wav.mul(alpha.get(k-bsIdx))
										.add(hk.mmul(Whv)).add(prevT.mmul(Wtv)).add(bv)));
			}
		}
//		vecV.putRow(eSize, DoubleMatrix.zeros(1, covSize));
...
//    		if(eSize-1>0) {
			...
			for(int k=bsIdx; k<t+1; k++) {
				if(lateDgV==null) {
					...
					deltaGV.putRow(k-bsIdx, lateDgs.getRow(k-bsIdx).mmul(att.Z.transpose())
							.add(lateDgV.getRow(k-bsIdx).mul(deriveExp(lateVecV.getRow(k-bsIdx))).mmul(Wvv.transpose())));
				}
			}
//    		}
	...
	private void calcWeightsGradient(Map<String, DoubleMatrix> acts, int lastT) {
    	...
    	for(int t=0; t<lastT+1; t++) { // no need to calculate the last one
			...
			for(int k=bsIdx; k<t+1; k++) {
				...
				dWav = dWav.add(deltaGVevV.getRow(k-bsIdx).mul(alpha.get(k-bsIdx)));

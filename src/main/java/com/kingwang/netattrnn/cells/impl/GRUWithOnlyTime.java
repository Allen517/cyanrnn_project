...
public class GRUWithOnlyTime extends Operator implements Cell, Serializable{
    ...
    public GRUWithOnlyTime(int inDynSize, int inFixedSize, int outSize, MatIniter initer) {
    ...
    public void bptt(Map<String, DoubleMatrix> acts, int lastT, Cell... cell) {
    	...
    	AttentionWithOnlyTime att = (AttentionWithOnlyTime)cell[0];
        ...
        for (int t = lastT; t > -1; t--) {
            ...
            for(int k=t; k<maxSize; k++) {
            	...
            	DoubleMatrix deltaGV = acts.get("dgV"+k);
    			DoubleMatrix vecV = acts.get("v"+k);
    			DoubleMatrix deltaGVevV = deltaGV.mul(deriveExp(vecV));
    			int bsIdx = Math.max(0, k-AlgConsHSoftmax.windowSize+1);
            	deltaH = deltaH.add(deltaS.mul(alpha.get(t-bsIdx))
            						.add(deltaGs.getRow(t-bsIdx).mmul(att.U.transpose()))
            						.add(deltaGVevV.getRow(t-bsIdx).mmul(att.Whv.transpose())));
            }
